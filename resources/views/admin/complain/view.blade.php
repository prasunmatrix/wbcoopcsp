    ...
    <div class="col-sm-7" >
      @if(isset($saleData->status))
            @if($saleData->status!=null || $saleData->status!='')
              ...
              <a onclick="return confirm('Are you sure ? Want to change the status!!')" href="{{ route('admin.complain.changestatus', [$saleData->id]) }}" title=""><span class="yellowBt">Ongoing</span></a>
  ...
  <div class="row">
    <div class="col-md-6">
      <div class="form-group">
        <a href="{{ route('admin.complain.list') }}" class="btn btn-info">Back</a>
      </div>
    </div>
    @if($saleData->status != '1')
    @if(\Auth::guard('admin')->user()->id == $saleData->reported_to)
